...
for batch in sorted(base.glob('*/batch_configuration.json')):
    ...
    row={'attempt':folder.name,'seed':config['seed'],'gamma':config['gamma'],'speed_min':config['speed_min'],'beta':config['beta_u'],
         'initial_x':config['initial_x'],'duration':config['horizon']*config['dt'],
         'activation':config['cp_activate_margin'],'dt':config['dt'],
         'waypoint':[config.get('reference_waypoint_x',-.65),config.get('reference_waypoint_y',.65)],
...
lines=['# V5 design attempts','','All percentages from completed pilots are design evidence, not held-out final-suite results.','','| Attempt | Status | Pretraining infeasible nodes | Worst pretraining margin |','|---|---|---:|---:|']
for r in rows:
